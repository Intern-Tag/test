...
function App () {
  ...
  const [MainTodoCheckedItems, setMainTodoCheckedItems] = useState({
    checked: false
  })
  ...
  const handleCheckSubtask = event => {
    ...
    //****hit the function */
    MainCheckBoxSubmit(id, event.target.checked)
  }

  //*****after setting the id and value , let submit to the backend to update the status of the marked todo task */
  function MainCheckBoxSubmit (MainTodoCheckedId, MainTodoCheckedValue) {
    console.log("to be ", MainTodoCheckedValue)
    handleMainCheckedBoxTodos(MainTodoCheckedId, MainTodoCheckedValue).then(
      res => {
        console.log(res)
        setReload(true)
      }
  ...
  const handleSubCheckSubtask = event => {
    ...
    //****hit the function */
    SubCheckBoxSubmit(id, event.target.checked)
  }

  //*****after setting the id and value , let submit to the backend to update the status of the marked sub todo task */
  function SubCheckBoxSubmit (SubTodoCheckedId, SubTodoCheckedValue) {
    handleSubCheckedBoxTodos(SubTodoCheckedId, SubTodoCheckedValue).then(
      res => {
        console.log(res)
        setReload(true)
      }
